---
title: "Dictionary Method with Keywords from Kevin Levay and Enns et al. 2015"
author: "Derek Burk"
output: pdf_document
---
...
write.csv(out, file="explicit and one-mention by year.csv", row.names = FALSE)
...
# Make a table of one mention by hand-coded category
one_mention_by_hand_coded <- table(d[ , .(one_mention, five_code)])
names(dimnames(one_mention_by_hand_coded)) <- NULL
one_mention_by_hand_coded <- one_mention_by_hand_coded[, c("explicit", "implicit", "relchanges", "releconomy", "irrelevant")]
rownames(one_mention_by_hand_coded) <- c("No keywords", "At least one keyword")

# Generate threshold (95th percentile of irrelevant articles)
# irrel_dist <- d[five_code=="irrelevant", .(all_pct=all_count/wtd_word_count, weight=weight)]
# setorder(irrel_dist, all_pct)
# irrel_cum_pct <- cumsum(irrel_dist$weight)/sum(irrel_dist$weight)
# pctile_95 <- approx(x=irrel_cum_pct, y=irrel_dist$all_pct, xout=0.95) # This is 0; only 3 of 467 irrelevant articles contain one of the keywords

# Calculate how many articles each keyword appears in
levay_kw_incidence <- matrix(sapply(sort(levay), function(x) sum(str_detect(remove_punctuation(tolower(d$text)), apply_word_boundaries(x)))), dimnames=list(sort(levay), "Number of articles"))

enns_kw_incidence <- matrix(sapply(sort(enns), function(x) sum(str_detect(remove_punctuation(tolower(d$text)), apply_word_boundaries(x)))), dimnames=list(sort(enns), "Number of articles"))

# Make correlation matrix of: pct enns, pct levay, pct all, pct explicit, pct exp or implicit, pct w/ one mention
corr_mtrx <- round(cor(cbind(levay_count_by_year$V1, enns_count_by_year$V1, all_count_by_year$V1, one_mention_by_year[,V1], pct_explicit_by_year$V1, pct_implicit_or_explicit_by_year$V1)), 2)
corr_names <- c("Levay", "Enns", "Combined", "One Mention", "Explicit", "(Exp/Imp)licit")
dimnames(corr_mtrx) <- list(corr_names, corr_names)
corr_mtrx[upper.tri(corr_mtrx, diag=TRUE)] <- "-"

```

The question guiding this analysis is whether a dictionary method, which classifies articles based only on the incidence of pre-selected keywords, gives us similar results to those obtained by hand-coding. One way to answer that question is by looking at whether the dictionary method and hand-coding classify the same articles as "inequality" articles. (The exact keywords used for the dictionary method can be found in the tables at the end of this document.) Table 1 shows the incidence of keywords (in keywords per 1,000 words) in articles according to hand-coded category.

 
```{r echo=FALSE, results="asis"}
kable(kw_count_by_code_table, caption="Weighted count of inequality keywords per 1,000 words, by hand-coded category")
```

Table 2 shows the number of articles with no keywords and the number with at least one keyword according to the hand-coded category. These results indicate that the keywords appear almost exclusively in articles hand-coded as "explicit".

```{r echo=FALSE, results="asis"}
kable(one_mention_by_hand_coded, caption="Number of articles containing zero versus at least one keyword, by hand-coded category")
```

Another way to compare the dictionary method to hand-coding is to compare the time trends in coverage of inequality that each method implies. Table 3 shows the correlation between the time trends implied by six different classification schemes, four based on the dictionary method, two on the hand-coding method. The first three methods construct the time trend as the incidence of keywords per 1,000 words in each year, using the Levay, Enns et al. 2015, and combined lists of keywords, respectively. The fourth method constructs the time trend as the percentage of articles in each year that contain at least one of the keywords from the combined list. And the last two methods construct the time trend as the percentage of articles hand-coded as "explicit", or as "explicit" or "implicit", respectively.

\pagebreak

```{r echo=FALSE, results="asis"}
kable(corr_mtrx, caption="Correlation of time-trends implied by six different classification methods")
```

The dictionary-based and hand-coding-based methods with the highest time-trend correlation are the "one mention" dictionary method and the "explicit" hand-coding method, which have a correlation of 0.59. Figure 1 graphs the time-trends implied by each of these methods.

```{r echo=FALSE, results="asis"}
#windowsFonts(A=windowsFont("CMU Serif"))
plot(pct_explicit_by_year, type="l", col="purple", family="serif", main="Figure 1. Percent of articles with at least one keyword, and percent \nof articles hand-coded as \"explicit\", 1980-2012", ylab="%", xlab="Year", lwd=2, ylim=c(0, 43))
text(2009, 24, "explicit", col="purple", family="serif")
lines(one_mention_by_year, lwd=2, col="grey")
text(2010, 0, "one-mention", col="grey", family="serif")
```

```{r echo=FALSE, results="asis"}
kable(levay_kw_incidence, caption="Total number of articles (out of 1,253) in which each of Levay's keywords appeared")
```


```{r echo=FALSE, results="asis"}
kable(enns_kw_incidence, caption="Total number of articles (out of 1,253) in which each of Enns et al.'s keywords appeared")
```